...
/// Practice npc
    NPC*dummy;
    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_OCELLUS, Arena::WIDTH/2, 0, Arena::WIDTH/2, Arena::HEIGHT+100);
    stageShipList.insert( std::make_pair( 1, dummy ));

/// Three npcs
    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_RAY, Arena::WIDTH-100, 0, Arena::WIDTH-100, Arena::HEIGHT+100);
    stageShipList.insert( std::make_pair( 4*Timer::FPS, dummy));

    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_ANTLION, Arena::WIDTH/2, 0, Arena::WIDTH/2, Arena::HEIGHT+100);
    stageShipList.insert( std::make_pair( 4*Timer::FPS, dummy));

    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_RAY, 100, 0, 100, Arena::HEIGHT+100);
    stageShipList.insert( std::make_pair( 4*Timer::FPS, dummy));

/// Four npcs
    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_OCELLUS, 0, 96, Arena::WIDTH+100, 192);
    stageShipList.insert( std::make_pair( 10*Timer::FPS, dummy));

    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_RAY, Arena::WIDTH, 192, 0, 384);
    stageShipList.insert( std::make_pair( 10*Timer::FPS, dummy));

    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_RAY, 0, 288, Arena::WIDTH+100, 576);
    stageShipList.insert( std::make_pair( 12*Timer::FPS, dummy));

    dummy = new NPC();
    dummy->NPC::Initialize(NPC::HULL_NPC_OCELLUS, Arena::WIDTH, 384, 0, 768);
    stageShipList.insert( std::make_pair( 12*Timer::FPS, dummy));
}
